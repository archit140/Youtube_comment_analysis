...
from project_utils import INTERIM_DATA_DIR, PROJECT_ROOT, setup_logger
from src.data.data_ingestion import load_params

logger = setup_logger("model_building", "model_building_errors.log")
...
def apply_tfidf(train_data: pd.DataFrame, max_features: int, ngram_range: tuple) -> tuple:
    ...
    try:
        ...
        with open(get_root_directory() / "tfidf_vectorizer.pkl", 'wb') as f:
...
def main():
    try:
        ...
        save_model(best_model, str(root_dir / "lgbm_model.pkl"))

    except Exception as e:
        logger.exception("Failed to complete the feature engineering and model building process: %s", e)
        raise
